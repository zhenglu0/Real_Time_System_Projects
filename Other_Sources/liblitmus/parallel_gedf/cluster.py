...
def partition(inputname, rawinfo, info, corenum, balance):
    ...
    #GEDF bound
	if balance == -1:
		(sched, corestr, outinfo) = GEDF(info, prognum, corenum, balance)
	elif balance != 3 and balance != 7:
	...
	if outinfo != []:
		...
		for each in rawinfo[1]:#[i][1:]:
			if int(each[5]) > int(periodmax1):
				periodmax1 = each[5]
				periodmax2 = each[6]
			elif int(each[5]) == int(periodmax1) and int(each[6]) > int(periodmax2):
				periodmax2 = each[6]
		#print outinfo
		for i in range(0, len(outinfo)):
			if rawinfo[0][i][0] != outinfo[i][0]:
				...
				#print outinfo[i][6],outinfo[i][7],exp_version
				if name:
				...
				for each in rawinfo[1][i][1:]:
					findrelease += 1
					if findrelease == 10 and balance == -1:
						#each = str(int(each) + (97-outinfo[i][5])*100000 )
						each = periodmax2
					elif findrelease == 9 and balance == -1:
						each = periodmax1
					line = line + each + ' '
				line = line.strip(' ')+'\n'
