...
//SG
//#include <omp.h>
//SG: Added the next four lines
#include <unistd.h>
#include <cilk_api.h>
#include <sstream>
#include <cilk_api_linux.h>
#include <iostream>
#include "task.h"
...
{ 
	...
	RT_GOMP_TASK_MANAGER_CILK_WORKERS_ERROR
};
...
int main(int argc, char *argv[])
{
	...
	//SG: Tasks are now bound to their assigned cores in clustering_launcher.cpp.
	...
	*/
	...
	//SG: Set the number of workers (i.e.  threads) in Cilk Plus
	//Must convert core range to char because __cilkrts_set_param(const char*, const char*)
	int num = last_core - first_core + 1;
	std::ostringstream convert;
	convert << num;
	ret_val = __cilkrts_set_param("nworkers", convert.str().c_str());
	if (ret_val != 0)
	  {
	    fprintf(stderr, "ERROR: Failed to set the number of cilk workers for task %s", task_name);
	    kill(0, SIGTERM);
	    return RT_GOMP_TASK_MANAGER_CILK_WORKERS_ERROR;
	  }
	...
	std::cout << deadlines_missed << " " << num_iters << " " << ( last_core - first_core + 1 ) << " " << max_period_runtime;
	//std::cout << deadlines_missed << " " << num_iters << " " << ( last_core - first_core + 1 ) << " " << maxiteration;
	fflush(stdout);
