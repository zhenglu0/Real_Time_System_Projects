	...
	for i in range(0, len(lowinfo)):
		...
		while count < lowcorenum:
			...
			if corestat[lowcore[curcore]][0] == 0:
				...
				sumutil += (preempt*27*1000.0+1-thr)
...
##omp	while theslack < (8*25.0*work/span+120)*1000.0 and numcore-tmp<10:
			if option == 7 and exp_version == 1:
				...
				while theslack < (8+6*math.log(numcore,2))*1000 and numcore-tmp<10:
					numcore+=1
					theslack = period-span-1.0*(work - span)/(numcore*0.95)
				if numcore-tmp>= 4:
					return 2, [], []
				...
				numseg = overheadinfo[i][5]
                        	while theslack < (27+(0.8+0.6*math.log(numcore,2))*numseg)*1000 and numcore-tmp<10:
					numcore+=1
					theslack = period-span-1.0*(work - span)/numcore
				if numcore-tmp>= 4:
					return 2, [], []
		#only for austen
			elif option == 12 and exp_version == 0:
				theslack = period-span-1.0*(work - span)/(numcore*0.95)
				numseg = overheadinfo[i][5]
				#while theslack < 1000.0*((1*numcore+20)+(3.8*numcore+23)+27) and numcore-tmp<10:
				while theslack < 1000.0*((5*numcore+50)+27) and numcore-tmp<10:
					numcore+=1
					theslack = period-span-1.0*(work - span)/(numcore*0.95)
				if numcore-tmp>= 4:
					return 2, [], []
				...
				theslack = period-span-1.0*(work - span)/(numcore*0.95)
				numseg = overheadinfo[i][5]
				while theslack < 1000.0*((0.01*numcore+0.20)*(work/span/numcore)*numseg+(0.38*numcore+2.3)*numseg) and numcore-tmp<10:
					numcore+=1
					theslack = period-span-1.0*(work - span)/(numcore*0.95)
				if numcore-tmp>= 4:
					return 2, [], []
...
#GEDF bound
#fake core assignment
def GEDF(overheadinfo, prognum, corenum, option):
    info = []
    for each in overheadinfo:
        info.append(copy.deepcopy(each[0:5]))
    overheadinfo.sort(sortutil)
    ...
    #from high task, set all core to 1
    utilsum = 0
    criticallpathflag = 0
    for i in range(0, len(info)):
        newprog = info[i]+[i+1, 0, corenum-1]
        outinfo.append(newprog)
        corestr[1].append(newprog)
        utilsum += info[i][2]
        if 4*info[i][4] > info[i][1]:
            criticallpathflag = 1

    if utilsum > corenum*0.25 or criticallpathflag == 1:
        #print "not core for high tasks"
        return 1, corestr, outinfo
    else:
        return 0, corestr, outinfo
