...
/* A slightly more complex example of the RT OMP system. This program creates
 * a task with two segments.
*/
...
//Specify the range of processors to execute on. Includes endpoints.
unsigned seg1_start_cpu = 12;
unsigned seg1_end_cpu   = 17;
unsigned seg2_start_cpu = 18;
unsigned seg2_end_cpu   = 23;
...
int main(int argc, char* argv[]){
  ...
  */
  ...
  for(unsigned i = seg1_start_cpu; i <= seg1_end_cpu; i++)
    task.addStrandToLastSeg( small_work, NULL, i); // Adds work to be done to
                                                   // the above segment.

  task.addSegment( seg_length, seg_release, 10);

  for(unsigned i = seg2_start_cpu; i <= seg2_end_cpu; i++)
    task.addStrandToLastSeg( small_work, NULL, i); // Adds work to be done to
